...
public partial class MainWindow : Window
{
    ...
    public async void PostNowButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        // Stop if both services are disabled
        ...
        {
            discordResult = await DiscordService.SendPayloadAsync(SelectedMovie.DiscordPayload());
        }

        // Try to send the movie to Slack
        if (Settings.Default.SendToSlack && !string.IsNullOrEmpty(Settings.Default.SlackWebHook))
        {
            slackResult = await SlackService.SendPayloadAsync(SelectedMovie.SlackPayload(), SelectedMovie.Title, SelectedMovie.YTVideoURL);
        }
