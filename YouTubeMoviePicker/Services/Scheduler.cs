...
internal class Scheduler
{
    ...
    private async void ExecutePostingTask(object state)
    {
        ...
        {
            discordResult = await DiscordService.SendPayloadAsync(nextMovie.DiscordPayload());
        }

        // Try to send the movie to Slack
        if (Settings.Default.SendToSlack && !string.IsNullOrEmpty(Settings.Default.SlackWebHook))
        {
            slackResult = await SlackService.SendPayloadAsync(nextMovie.SlackPayload(), nextMovie.Title, nextMovie.YTVideoURL);
        }
